...
function renderDeptos() {
  ...
  if(PINOS.nombre){
    ...
    pc.innerHTML='<div class="card" style="border-left:3px solid #7c3aed;border-radius:0 12px 12px 0"><div class="flex" style="justify-content:space-between;margin-bottom:.75rem;flex-wrap:wrap;gap:8px"><div class="flex gap-8" style="cursor:pointer" onclick="verPinos()"><div style="background:#f3e8ff;border-radius:8px;padding:4px 10px;font-size:18px;min-width:40px;text-align:center">🏠</div><div><div style="font-weight:600;font-size:14px">'+esc(PINOS.nombre)+'  <span style="font-size:11px;background:#f3e8ff;color:#6b21a8;border:1px solid #d8b4fe;border-radius:4px;padding:2px 7px">Los Pinos</span></div><div class="text-muted">'+(PINOS.dur||'1 año')+' · '+fmt(PINOS.monto||22000)+'/mes'+(pfin?' · '+pfin:'')+'</div></div></div><div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">'+pbadge+pdep+'<button class="btn btn-sm" onclick="verPinos()"><i class="ti ti-edit"></i> Editar</button></div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px;font-size:12px;border-top:1px solid #eee;padding-top:.75rem"><div><span class="text-muted">Teléfono</span><br>'+ptel+'</div><div><span class="text-muted">Correo</span><br><strong>'+esc(PINOS.email||'—')+'</strong></div><div><span class="text-muted">Aval</span><br><strong>'+esc(PINOS.aval||'—')+'</strong></div></div></div>';
  } else {
